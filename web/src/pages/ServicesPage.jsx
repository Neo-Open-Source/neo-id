...
import { getProfile, adminListClients, adminCreateClient, adminUpdateClient, adminDeleteClient } from '../api/endpoints'
    ...
    <AppLayout title="Neo ID" navItems={[{ label: '← Dashboard', onClick: () => navigate('/dashboard') }]}>
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 760 }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 600 }}>OIDC Clients</Typography>
            <Typography variant="body2" color="text.secondary">Manage your OpenID Connect clients</Typography>
          </Box>
        ...
        {loading ? (
          ...
          <Stack spacing={2}>
            {clients.map(c => (
              <ClientCard key={c.site_id || c.client_id} client={c}
                onEdit={c => { setEditClient(c); setDialogOpen(true) }}
                onDelete={onDelete} />
            ))}
          </Stack>
        )}
      ...
      <ClientDialog open={dialogOpen} client={editClient} onClose={() => setDialogOpen(false)}
        onSaved={async () => { setDialogOpen(false); notify('success', editClient ? 'Updated' : 'Created'); await load() }} />
    </AppLayout>
  )
}
